...
- **Boot-time recovery window — recover a device stranded before `ota.poll()`, over the radio, no USB.** On every boot with no update pending, `boot.py` listens for `OTA_BOOT_LISTEN_MS` (new device setting, default `1000`, `0` disables) after `repair()`/`trial()` have healed the tree.
  ...
  - **Cost:** every boot that is not applying an update is ~1 s longer (plus one `Urst` transport instantiation), boot-loop iterations included. Tunable via `OTA_BOOT_LISTEN_MS`; `0` removes the window and the recovery path with it. Documented in `docs/protocol.md` §2.4 and `docs/architecture.md`.
