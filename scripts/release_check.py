...
def _require_clean_worktree() -> None:
    ...
    if result.stdout.strip():
        raise ReleaseCheckError(
            "The worktree is not clean. Commit or stash changes before "
...
def inspect_artifact(path: Path) -> None:
    ...
    if forbidden_names:
        raise ReleaseCheckError(
            f"Forbidden generated or private files in {path.name}: "
            f"{forbidden_names}"
        )
...
def smoke_test_install(
    ...
    *,
    ...
    if urst_source is not None:
        ...
        print(
            "Using a local URST source for preflight only; run without "
            "--urst-source before publishing OTAmpy.",
            flush=True,
        )
