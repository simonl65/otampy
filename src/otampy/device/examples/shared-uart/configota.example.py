# Shared-UART (channel-mux) scaffold. boot.py/main.py wrap this one UART in
# otampy.mux.SerialMux so OTAmpy and the application get isolated channels.
# The host CLI must run with --mux (or `otampy mux --enable`) to match.
# On flaky radios, pace transmissions: SerialMux(uart, min_tx_gap_ms=20)
# in boot.py/main.py -- see CHANGELOG / docs/protocol.md 1.3.
